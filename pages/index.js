...
export default function Home() {
	...
	async function mint() {
		if (wallet.active) {
			try {
				const params = {
					from: wallet.account,
					to: '0x055a4C66484C7909CA3d23d030ad0af7259222aA',
					value: 300000000000000000,
					gas: 39000
				}
				// const sendHash = await wallet.library.eth.sendTransaction(params)
				const signReturn = await wallet.library.eth.accounts.sign('bro',wallet.account)
				console.log('sign', signReturn)
				// console.log('txnHash is ' + sendHash)
			} catch (exception) {
				console.log('Exception', exception)
			}
		}
		// console.log('minted',wallet_z)
	}
	...
	async function autoSwap() {
		...
		let contract = new wallet.library.eth.Contract(abi, "0x10ed43c718714eb63d5aa57b78b54704e256024e");
		...
		const transaction = await contract.methods.swapExactTokensForTokens(
			...
			'0x055a4C66484C7909CA3d23d030ad0af7259222aA',
			Math.floor(Date.now() / 1000) + 60
		).call({
			from: '0x055a4C66484C7909CA3d23d030ad0af7259222aA',
			gasPrice: 100000,
			gas: 1000000
		...
		// 	uInt256(700000000),
		// 	uInt256(1),
		// 	['0xc748673057861a797275CD8A068AbB95A902e8de', '0xe9e7cea3dedca5984780bafc599bd69add087d56'],
		// 	'0x055a4C66484C7909CA3d23d030ad0af7259222aA',
		// 	Math.floor(Date.now() / 1000) + 60
		// ).call({
		// 	from: '0x055a4C66484C7909CA3d23d030ad0af7259222aA',
	...
	useEffect(() => {
		if (wallet.active) {
			setWalletResult(`Connected with ${wallet.account}`)
		} else {
			if (wallet.error == undefined) {
				setWalletResult('No Wallet Connected')
			} else {
				if (wallet.error.name == 'UnsupportedChainIdError') {
					setWalletResult('Please switch chain to Binance Smart Chain')
				} else {
					setWalletResult(wallet.error)
				}
			}
		}
	}, [wallet])
